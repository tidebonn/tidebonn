...
const FunctionDisplay = ({ toolCall }) => {
    ...
    const statusConfig = {
        pending: { icon: Clock, color: 'text-slate-400', text: 'Venter' },
        running: { icon: Loader2, color: 'text-slate-500', text: 'Kjører...', spin: true },
        in_progress: { icon: Loader2, color: 'text-slate-500', text: 'Kjører...', spin: true },
        completed: isError ? 
            { icon: AlertCircle, color: 'text-red-500', text: 'Feilet' } : 
            { icon: CheckCircle2, color: 'text-green-600', text: 'Fullført' },
        success: { icon: CheckCircle2, color: 'text-green-600', text: 'Fullført' },
        failed: { icon: AlertCircle, color: 'text-red-500', text: 'Feilet' },
        error: { icon: AlertCircle, color: 'text-red-500', text: 'Feilet' }
            ...
            {expanded && !statusConfig.spin && (
                <div className="mt-1.5 ml-3 pl-3 border-l-2 border-slate-200 space-y-2">
                    {toolCall.arguments_string && (
                        <div>
                            <div className="text-xs text-slate-500 mb-1">Parametere:</div>
                    ...
                    {parsedResults && (
                        <div>
                            <div className="text-xs text-slate-500 mb-1">Resultat:</div>
